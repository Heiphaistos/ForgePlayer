...
// SwsContext holds no thread-affine state (just scaling tables) — safe to move
// ownership across threads as long as it isn't accessed concurrently, which
// omni-core never does (each instance lives on exactly one decode thread).
